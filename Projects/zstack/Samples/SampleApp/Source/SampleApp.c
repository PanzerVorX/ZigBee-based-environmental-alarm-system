...
#define FMQ_PIN P0_4
//#define LEDW_PIN P1_1
//#define LEDQ_PIN P1_0
...
/****************************************************************************
* 名 称: DelayMS()
* 功 能: 以毫秒为单位延时 16M 时约为 535,32M 时要调整,系统时钟不修改默认为
16M
...
{
uint16 reading = 0;
ADCCFG |= 0x40; /* Enable channel */
/* writing to this register starts the extra conversion */
ADCCON3 = 0x86;// 0x87;
/* Wait for the conversion to be done */
while (!(ADCCON1 & 0x80));
/* Disable channel after done conversion */
ADCCFG &= (0x80 ^ 0xFF); //按位异或。如 1010^1111=0101（二进制）
/* Read the result */
reading = ADCL;
  ...
  switch ( pkt->clusterId )
  {
    case SAMPLEAPP_P2P_CLUSTERID:
      ...
      if(pkt->cmd.DataLength==6)
      {
        ...
        HalUARTWrite(0, "\n", 1);// 回车换行
        wdVar=(test[0]-48)*10+(test[1]-48);
        //UartSendString(test,2);
        //osal_memcpy(test,pkt->cmd.Data,3); 
        //UartSendString(test,3);
        if(wdVar>30)
        {
          P1_1=1;
          //LEDW_PIN=1;
          //FMQ_PIN=1;
          HalUARTWrite(0, "Abnormal temperature",21);
          HalUARTWrite(0, "\n", 1); // 回车换行
        }
        else
        {
      ...
      {
        HalUARTWrite(0, "MQ2:", 4); //提示接收到数据
        HalUARTWrite(0, pkt->cmd.Data, pkt->cmd.DataLength); //输出接收到的数据
        HalUARTWrite(0, "\n", 1); // 回车换行
        qtVar=(test[0]-48)*100+(test[1]-48)*10+(test[2]-48);
         //UartSendString(test,3);
        //osal_memcpy(test,pkt->cmd.Data,3); 
        //UartSendString(test,3);
        if(qtVar>40)
...
void SampleApp_Send_P2P_Message( void )
{
  ...
  GasData = ReadGasData(); //读取烟雾传感器引脚上的 ad 转换值，并没有换算成能表示烟雾浓度的值。演示如何使用 2530 芯片的 AD 功能，更具体在组网中给出
  ...
  TxBuf[3] = '\n';
  TxBuf[4] = 0;
  //UartSendString(TxBuf, 4); //通过串口发出数据，波特率是 115200 8N1
  //DelayMS(1000); //延时函数
    
  
  
  HalUARTWrite(0,TxBuf, 5);//串口显示
  ...
  { 
    ...
    HalUARTWrite(0,"GAS:security\n", 13);                 //串口显示
    HalLcdWriteString( "GAS:security\n", HAL_LCD_LINE_3 ); //LCD显示 
  }  
  else 
  {  
    ...
    {     
      ...
      HalUARTWrite(0,"GAS:Got bad Air\n",16);                 //串口显示
      HalLcdWriteString( "GAS:Got bad Air", HAL_LCD_LINE_3 ); //LCD显示 
    }
  }
